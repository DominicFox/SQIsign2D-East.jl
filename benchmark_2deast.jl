...
function main()
    # Set some defaults in case none are given
    iterations = 100
    num_keys = 50
    warmup_runs = 15
    mode = 0
    ...
    # Helper function to convert complex Nemo objects into padded hashes for Rust (This is a workaround for storing and recovering keys in CSV for later if needed)
    ...
    # =========================================================
    # PHASE 2: MODE 1 (Fixed Key, Random Messages)
    # =========================================================
    elseif mode == 1
        ...
        # Roll all random 32-byte messages
        messages = [rand(UInt8, 32) for _ in 1:total_runs]

        # Execute signatures against random messages
        for i in 1:total_runs
            ...
            # Using key_index 0 to match C behaviour
            push!(telemetry, TelemetryRow(0, batch_keygen_cycles, st1 - st0, vt1 - vt0))
        end

        # Dump variables to binary files for Rust to read
    ...
    # =========================================================
    # PHASE 3: MODE 2 (Fixed Message, Random Keys)
    # =========================================================
    elseif mode == 2
        ...
        # Loop generating a random key and signing/verifying the static message with it
